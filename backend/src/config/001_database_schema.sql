...
Constraint ck_roles_name check(role_name in ('user','employee','admin','jury'))
);	
...
CREATE OR REPLACE FUNCTION public.hash_admin(adm_name text,email text, pass text)
RETURNS TEXT
SECURITY DEFINER
...
p.product_type,
p.producer_name, 
concat(p.in_stock::integer,' ',p.unit) as quantity, 
p.arrival_date,
p.expiration_date,
CASE 
    WHEN p.in_stock > 0 THEN 'arrived' 
    ELSE 'empty' 
END as status from products p;
...
CONSTRAINT ck_reservations_no_persons CHECK (no_persons > 0),
CONSTRAINT ck_reservations_total_price CHECK (total_price >= 0),
CONSTRAINT ck_reservations_event_date CHECK (event_date >= date_res_made)
);
...
CREATE TABLE IF NOT EXISTS reservation_addresses (
    ...
    CONSTRAINT fk_reservation_addresses_reservation
        FOREIGN KEY (res_id)
...
Create table reviews(
	res_id BIGINT PRIMARY KEY references reservations(res_id), 
